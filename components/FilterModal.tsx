...
const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose, currentFilters, onApplyFilters, initialFilters }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-11/12 max-w-sm m-4 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        ...
        <main className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center justify-between gap-4">
              <input type="number" value={tempFilters.ageRange[0]} onChange={e => handleAgeChange(0, e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg text-center" min="18" max="99" />
              <span className="text-gray-500">-</span>
              <input type="number" value={tempFilters.ageRange[1]} onChange={e => handleAgeChange(1, e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg text-center" min="18" max="99" />
            </div>
          </div>
